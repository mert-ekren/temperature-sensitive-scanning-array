...
void let_there_be_tt(double TT){
	//PLL_Init();                           // set system clock to 50 MHz
	Nokia5110_Init();
	int integerPartTT = (int)TT;
	int fractionalPartTT = (int)((TT - integerPartTT) * 100);	
	Nokia5110_Clear();
	Nokia5110_SetCursor(0, 0);
	Nokia5110_OutString("TT:");
	Nokia5110_SetCursor(4, 0);
	Nokia5110_OutUDec(integerPartTT);
	Nokia5110_OutString(".");
	Nokia5110_OutUDec(fractionalPartTT);
}

void let_there_be_screen(uint16_t o_exs, double TT, double T, uint16_t ANG, uint16_t D){
  //PLL_Init();                           // set system clock to 50 MHz
  Nokia5110_Init();
	int integerPartTT = (int)TT;
	int fractionalPartTT = (int)((TT - integerPartTT) * 100);
	
	int integerPartT = (int)T;
	int fractionalPartT = (int)((T - integerPartT) * 100);
	...
	if (o_exs == 1){
		if (ANG-90 > -1){
			...
			Nokia5110_OutUDec(integerPartT);
			Nokia5110_OutString(".");
			Nokia5110_OutUDec(fractionalPartT);
		
			Nokia5110_SetCursor(0, 2);
			Nokia5110_OutString("ANG:");
			Nokia5110_OutUDec(ANG);
